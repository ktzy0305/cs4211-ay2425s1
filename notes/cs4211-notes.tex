...
\begin{frame}{CSP Syntax}
    \begin{enumerate}
        ...
        \item Choice: $a \rightarrow P \extchoice b \rightarrow Q$
        \item Interrupt Process: $P \interrupt e \rightarrow Q$
    \end{enumerate}

    \footnotetext{Note that in the lecture slides, the nabla symbol $\nabla$ is used for interrupt.}
\end{frame}
...
\begin{frame}{Prefix}
    \begin{itemize}
        ...
        \item Examples
        \begin{enumerate}
            \item $VMU = coin \rightarrow STOP$
            \item $SHORTLIFE = (beat \rightarrow (beat \rightarrow STOP)) = beat \rightarrow beat \rightarrow STOP$
            \item $VMS = coin \rightarrow choc \rightarrow STOP$
        \end{enumerate}
    \end{itemize}
\end{frame}
...
\begin{frame}{Sequential Composition ($P;Q$)}
    \begin{itemize}
        ...
        \item Let $SKIP = \tick \rightarrow STOP$.
...
\begin{frame}{Parallel Composition}
    \begin{itemize}
        ...
        \item Example: $(a \rightarrow P) \parallel[a] (c \rightarrow a \rightarrow Q)$
        \begin{itemize}
...
\begin{frame}{Interleaving}
    \begin{itemize}
        ...
        \item Example: $((a \rightarrow P) \interleave (c \rightarrow a \rightarrow Q))$
        \begin{itemize}
            \item One possible trace is $\trace{c, a, a}$, after which the process acts as $P \interleave Q$
            \begin{enumerate}
                \item $c$ from $c \rightarrow a \rightarrow Q$ is engaged, leaving us with $((a \rightarrow P) \interleave (a \rightarrow Q))$
                \item $a$ from $a \rightarrow P$ is engaged, leaving us with $(P \interleave (a \rightarrow Q))$
                \item $a$ from $a \rightarrow Q$ is engaged, leaving us with $P \interleave Q$
            \end{enumerate}
            \item Another possible trace is $\trace{a, c, a}$, after which the process acts as $P \interleave Q$
            \begin{enumerate}
                \item $a$ from $a \rightarrow P$ is engaged, leaving us with $(P \interleave (c \rightarrow a \rightarrow Q))$
                \item $c$ from $c \rightarrow a \rightarrow Q$ is engaged, leaving us with $(P \interleave (a \rightarrow Q))$
                \item $a$ from $a \rightarrow Q$ is engaged, leaving us with $P \interleave Q$
            \end{enumerate}
        \end{itemize}
    \end{itemize}
...
\begin{frame}{Choice}
    \begin{itemize}
        \item In a general choice, $(a \rightarrow P) \extchoice (b \rightarrow Q)$, the process begins with both events \textbf{a} and \textbf{b} enabled.
        ...
        \item Example: $(a \rightarrow P) \extchoice (c \rightarrow a \rightarrow Q)$
        \begin{itemize}
            \item If the first event is \textbf{a}, after which the process acts as $P$.
            \item If the first event is \textbf{c}, after which the process acts as $a \rightarrow Q$.
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        \item The interrupt process $P \interrupt e \rightarrow Q$ behaves as \textbf{process P} until the first occurrence of \textbf{event e} which then the control passes to \textbf{process Q}. 
        \item When coding the specification, the keyword \textbf{interrupt} is used instead of the symbol $\interrupt$. 
...
\begin{itemize}
    ...
    \item In VMC, the \textbf{coin} event has to be engaged first before we can engage $choc \rightarrow VMC$
...
\begin{itemize}
    ...
    \item After $\trace{coin, bisc}$ has occurred, the system would be stuck at $VMC \parallel choc \rightarrow CHOCLOV$.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item The system will be stuck at $choc \rightarrow off \rightarrow VMH \parallel CUST$ and no event can be engaged.
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}{Laws for Concurrency}
    ...
    \end{enumerate}
    \begin{itemize}
        \item Law 4A: $(c \rightarrow P) \parallel (c \rightarrow Q) = c \rightarrow (P \parallel Q)$
        \item Law 4B: $(c \rightarrow P) \parallel (d \rightarrow Q) = STOP$ if $c \neq d$
        \item Law 5A: $(a \rightarrow P) \parallel (c \rightarrow Q) = a \rightarrow (P \parallel (c \rightarrow Q))$
        \item Law 5A: $(c \rightarrow P) \parallel (b \rightarrow Q) = b \rightarrow ((c \rightarrow P) \parallel Q)$
        \item Law 6: $(a \rightarrow P) \parallel (b \rightarrow Q) = a \rightarrow (P \parallel (b \rightarrow Q)) \extchoice b \rightarrow((a \rightarrow P) \parallel Q)$
    \end{itemize}
\end{frame}
...
\end{lstlisting}
\begin{itemize}
    \item $\parallel x : \{1 \upto n\}$ @ P(x) is equivalent to $P(1) \parallel \ldots \parallel P(n)$
