...
\begin{frame}{Recap on Quantifiers}
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \item $\forall n : \nat \spot n > 0$
            \item In general, $\forall x : X \spot P(x)$ abbreviates $P(a) \wedge P(b) \wedge P(c) \wedge \ldots$
        \end{itemize}
    ...
    \begin{block}{Differences between Mathematical Notations and Z Specification}
        \begin{itemize}
            ...
            \end{itemize}
            \item In Z Specification, $\spot$ means "such that" when writing predicates.
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{Operational Semantics: Example \#1}
    ...
    \begin{block}{Operation Semantics}
        Step 1. $VMS \xrightarrow{coin} (choc \then VMS \extchoice bisc \then VMS)$ (by rule prefixing)\\ 
        Step 2. $(choc \then VMS \extchoice bisc \then VMS) \xrightarrow{choc} VMS$ (by rule choice 1)\\
        Step 2. $(choc \then VMS \extchoice bisc \then VMS) \xrightarrow{bisc} VMS$ (by rule choice 2)
    \end{block}
...
#assert Patterson() reaches goal; // Should be not valid
#assert Patterson() |= <> cs.0; // Should be not valid
#assert Patterson() |= [] (update.0.1 -> <> cs.0) // Should be valid
\end{lstlisting}
\end{frame}
